...
from dynamic_network_architectures.building_blocks.unet_decoder import UNetDecoder, UNetDecoderMultiEncoders
from dynamic_network_architectures.building_blocks.helper import convert_conv_op_to_dim


class PlainConvUNetMultiEncoder(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
        ...
        self.decoder = UNetDecoderMultiEncoders(self.encoders, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=nonlin_first)
    ...
    def forward(self, x):
        ...
        for s in range(len(skips_from_encoders[0])):
            skips.append(torch.stack([skips_from_encoders[e][s] for e in range(self.num_encoders)]).sum(dim=0))
